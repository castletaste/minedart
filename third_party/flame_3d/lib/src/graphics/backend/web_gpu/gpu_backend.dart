...
base class GpuBackend extends base.GpuBackend {
  ...
  late final _WebGpuSampledTexture _defaultTexture;

  late final GPUTextureView _defaultTextureView = _defaultTexture
      .resolveFallbackView();
  ...
  final Map<_PipelineKey, _ResolvedWebGpuPipeline> _pipelineCache = {};

  @override
  GpuBindCacheCounters get lastBindCacheCounters => _frame.lastCounters;
  ...
  _ResolvedWebGpuPipeline _resolvePipeline(
    ...
    return _pipelineCache.putIfAbsent(
      ...
      () {
        final raw = _device.createRenderPipeline(
        ...
        final hasTargetGroup = pipeline.slotGroups.any(
          (plan) => plan.group == webGpuBindCacheTargetGroup,
        );
        return _ResolvedWebGpuPipeline(
          raw,
          webGpuBindCacheEnabled && hasTargetGroup
              ? raw.getBindGroupLayout(webGpuBindCacheTargetGroup)
              : null,
        );
      },
    );
  ...
  _WebGpuSampledTexture _solidTexture(int argb) {
    final texture = _device.createTexture(
      GPUTextureDescriptor(
    ...
    return _WebGpuSampledTexture(_queue, texture, 1, 1);
  }
...
class _WebGpuPipeline implements GpuPipeline {
  _WebGpuPipeline(this._vertex, this._fragment)
    : slotGroups = _createSlotGroups(_vertex.slots);
  ...
  /// Immutable group/binding-sorted plan retained for this pipeline lifetime.
  final List<_WebGpuSlotGroup> slotGroups;

  static List<_WebGpuSlotGroup> _createSlotGroups(
    Map<String, WebShaderSlot> reflectedSlots,
  ) {
    final sorted = reflectedSlots.values.toList()
      ..sort((a, b) {
        final byGroup = a.group.compareTo(b.group);
        return byGroup != 0 ? byGroup : a.binding.compareTo(b.binding);
      });
    final groups = <_WebGpuSlotGroup>[];
    var index = 0;
    while (index < sorted.length) {
      final group = sorted[index].group;
      final slots = <WebShaderSlot>[];
      while (index < sorted.length && sorted[index].group == group) {
        slots.add(sorted[index]);
        index++;
      }
      groups.add(_WebGpuSlotGroup(group, List.unmodifiable(slots)));
    }
    return List.unmodifiable(groups);
  }
}

class _WebGpuSlotGroup {
  const _WebGpuSlotGroup(this.group, this.slots);

  final int group;
  final List<WebShaderSlot> slots;
}

class _ResolvedWebGpuPipeline {
  const _ResolvedWebGpuPipeline(this.raw, this.targetGroupLayout);

  final GPURenderPipeline raw;
  final GPUBindGroupLayout? targetGroupLayout;
}
...
class _WebGpuSampledTexture implements GpuTexture {
  _WebGpuSampledTexture(this._queue, this.raw, this._width, this._height);
  ...
  final WebGpuTextureViewLifetime<GPUTextureView> _viewLifetime =
      WebGpuTextureViewLifetime<GPUTextureView>();

  GPUTextureView resolveFallbackView() {
    return _viewLifetime.resolveUntracked(() => raw.createView());
  }
...
class _WebGpuFrame implements base.GpuFrame {
  ...
  final WebGpuBindCachePolicy _bindCache = WebGpuBindCachePolicy();

  GpuBindCacheCounters get lastCounters => _bindCache.lastCounters;

  bool get bindCacheEnabled => _bindCache.enabled;
  ...
  (GPUBuffer, int) _uploadUniform(ByteData data) {
  ...
  WebGpuUniformAllocation resolveUniformUpload({
    required _WebGpuUniformSlot slot,
    required ByteData data,
    required Object? bindingIdentity,
    required int bindingRevision,
  }) {
    final reflected = slot.slot;
    return _bindCache.resolveUniformUpload(
      ...
      upload: () {
        final (buffer, offset) = _uploadUniform(data);
  ...
  GPUTextureView resolveTextureView({
    required int group,
    required _WebGpuSampledTexture texture,
  }) {
    return _bindCache.resolveTextureView(
      group: group,
      lifetime: texture._viewLifetime,
      create: () => texture.raw.createView(),
    );
  }

  GPUBindGroup resolveBindGroup({
    required GPURenderPipeline pipeline,
    required GPUBindGroupLayout layout,
    required int group,
    required List<WebGpuBindResourceKey> resources,
    required GPUBindGroup Function() create,
  }) {
    return _bindCache.resolveBindGroup(
  ...
  base.GpuRenderPass beginRenderPass(
    ...
    final pass = encoder.beginRenderPass(
      GPURenderPassDescriptor(
        // Swapchain and depth attachment views are frame/pass-local. They are
        // deliberately never routed through sampled-texture view caching.
        colorAttachments: [
...
class _WebGpuRenderPass implements base.GpuRenderPass {
  ...
  late _ResolvedWebGpuPipeline _renderPipeline;

  int _indexCount = 0;

  // Bindings accumulated for the current draw, keyed by `_key(group, binding)`.
  final Map<int, (GPUBuffer buffer, int offset, int size, int revision)>
  _uniforms = {};
  ...
  void bindPipeline(GpuPipeline pipeline, CullMode cullMode) {
    ...
    _pass.setPipeline(_renderPipeline.raw);
  }
  ...
  void bindUniform(
    ...
    final webSlot = slot as _WebGpuUniformSlot;
    final reflected = webSlot.slot;
    final allocation = _frame.resolveUniformUpload(
      slot: webSlot,
      data: data,
      bindingIdentity: bindingIdentity,
    ...
    _uniforms[_key(reflected.group, reflected.binding)] = (
      allocation.bufferToken as GPUBuffer,
      allocation.offset,
      allocation.size,
      allocation.revision,
    );
  ...
  void bindTexture(
    ...
    final reflected = slot.slot;
    _textures[_key(reflected.group, reflected.binding)] = _frame
        .resolveTextureView(group: reflected.group, texture: texture);
  }
  ...
  @override
  void draw() {
    if (!_frame.bindCacheEnabled) {
      _drawUpstreamPath();
      return;
    }

    final pipeline = _pipeline;
    final resolvedPipeline = _renderPipeline;

    for (final groupPlan in pipeline.slotGroups) {
      final group = groupPlan.group;
      final entries = <GPUBindGroupEntry>[];
      final cacheTarget =
          group == webGpuBindCacheTargetGroup && _frame.bindCacheEnabled;
      final resources = cacheTarget ? <WebGpuBindResourceKey>[] : null;
      for (final slot in groupPlan.slots) {
        if (slot.sizeInBytes != null) {
          final bound = _uniforms[_key(slot.group, slot.binding)];
          final buffer = bound?.$1 ?? _backend._zeroBuffer;
          final offset = bound?.$2 ?? 0;
          final size = bound?.$3 ?? slot.sizeInBytes!;
          final revision = bound?.$4 ?? 0;
          entries.add(
            GPUBindGroupEntry(
          ...
          resources?.add(
            WebGpuTextureBindKey(
              binding: slot.binding,
              samplerBinding: slot.samplerBinding!,
              viewToken: view,
              samplerToken: _backend._defaultSampler,
            ),
          );
        }
      }
      final layout = cacheTarget
          ? resolvedPipeline.targetGroupLayout!
          : resolvedPipeline.raw.getBindGroupLayout(group);
      GPUBindGroup createBindGroup() => _backend._device.createBindGroup(
        GPUBindGroupDescriptor(layout: layout, entries: entries),
      );
      _pass.setBindGroup(
        group,
        _frame.resolveBindGroup(
          pipeline: resolvedPipeline.raw,
          layout: layout,
          group: group,
          resources: resources ?? const <WebGpuBindResourceKey>[],
  ...
  /// Preserves the hosted 0.3.0 per-draw allocation/bind path for A/B builds.
  void _drawUpstreamPath() {
    final pipeline = _pipeline;
    final renderPipeline = _renderPipeline.raw;
    ...
    for (final MapEntry(key: group, value: slots) in groups.entries) {
      ...
      final layout = renderPipeline.getBindGroupLayout(group);
      _pass.setBindGroup(
        group,
        _frame.resolveBindGroup(
          ...
          layout: layout,
          group: group,
          resources: const <WebGpuBindResourceKey>[],
          create: () => _backend._device.createBindGroup(
            GPUBindGroupDescriptor(layout: layout, entries: entries),
          ),
        ),
      );
