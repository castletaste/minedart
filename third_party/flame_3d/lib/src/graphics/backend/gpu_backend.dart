...
abstract base class GpuBackend {
  ...
  /// Bind-cache counters for the most recently completed frame, when the
  /// active backend exposes them.
  GpuBindCacheCounters? get lastBindCacheCounters => null;
}
...
abstract interface class GpuRenderPass {
  ...
  /// can be reused afterwards. A retained binding may provide stable
  /// [bindingIdentity] and monotonic [bindingRevision] metadata; backends that
  /// do not use it ignore both values.
  void bindUniform(
    GpuUniformSlot slot,
    ByteData data, {
