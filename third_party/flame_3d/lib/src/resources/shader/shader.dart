...
class _UniformBinding extends _SlotBinding<ByteBuffer> {
  _UniformBinding(super.slot)
    : _bytes = Uint8List(slot.sizeInBytes!);

  @override
  ByteBuffer get resource => _bytes.buffer;

  final Uint8List _bytes;
  final Map<String, int> _memberOffsets = {};

  /// Stable for this retained binding's lifetime.
  Object get identity => this;

  /// Increments once when a setter changes one or more retained bytes.
  int revision = 0;
  ...
  void setField(String member, int? index, Float32List data) {
    final baseOffsetInBytes = _memberOffsets.putIfAbsent(
      member,
      () {
    ...
    final offsetInBytes = switch (index) {
      final i? =>
        baseOffsetInBytes + i * _std140ArrayStrideInBytes(data.length),
      _ => baseOffsetInBytes,
    };

    _setBytes(offsetInBytes, data);
  }
  ...
  void _setBytes(int offsetInBytes, Float32List data) {
    final incoming = data.buffer.asUint8List(
      data.offsetInBytes,
      data.lengthInBytes,
    );
    RangeError.checkValidRange(
      offsetInBytes,
      offsetInBytes + incoming.length,
      _bytes.length,
    );

    for (var i = 0; i < incoming.length; i++) {
      if (_bytes[offsetInBytes + i] != incoming[i]) {
        _bytes.setRange(
          offsetInBytes,
          offsetInBytes + incoming.length,
          incoming,
  ...
  /// boundary.
  static int _std140ArrayStrideInBytes(int elementFloats) {
    final strideInFloats = (elementFloats + 3) & ~3;
    return strideInFloats * Float32List.bytesPerElement;
  }
}
